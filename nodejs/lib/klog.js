...
(function() {
  var $args, $cmd, add_with_id, argv, changeBugState, cmd_add, cmd_append, cmd_close, cmd_delete, cmd_edit, cmd_html, cmd_init, cmd_reopen, cmd_search, cmd_view, date, editFile, exec, exit, fs, generateUID, getBugByUIDORNumber, getBugs, hook, md5, opts, pad, parseCommandLineArguments, print, randomUID, removeClog, usage;

  fs = require('fs');

  exec = require("child_process").exec;

  argv = require('optimist').alias('s', 'state').alias('e', 'editor').alias('t', 'type').alias('m', 'message').argv;
  ...
  */
  ...
  randomUID = function(o, t) {
    return exec('git config --get user.email', function(se, so, e) {
      var $r;
      $r = generateUID(so);
      return add_with_id(o, t, $r);
    });
  };

  generateUID = function($email) {
    var $uid;
  ...
  */
  ...
  cmd_add = function(args, type) {
    return randomUID(args, type);
  };

  add_with_id = function(args, type, $uid) {
    var $file, $template, $title, $type;
    if (args.length) {
      $title = args.join(" ");
    } else {
